...
constexpr uint32_t packColorF(float r, float g, float b, float a = 1.0f) {
    return packColor(static_cast<uint8_t>(r * 255.0f), static_cast<uint8_t>(g * 255.0f),
...
struct VertexElement {
    ...
    constexpr void set(const value_type& v) { std::memcpy(data_, &v, sizeof(v)); }
};
...
public:
    constexpr VertexBufferView(std::span<const std::byte> data, const VertexLayout& layout)
        : data_(data), layout_(layout) {}

    constexpr uint32_t vertexCount() const {
        return layout_.stride() > 0 ? static_cast<uint32_t>(data_.size()) / layout_.stride() : 0;
    }

    // 获取指定顶点属性的原始指针
    const std::byte* attributeData(uint32_t vertexIndex, VertexSemantic sem) const {
        ...
        if (offset + attr->size() > data_.size())
            return nullptr;
    ...
    Float3 readPosition(uint32_t idx) const { return read<Float3>(idx, VertexSemantic::Position); }

    // 快捷：读取打包颜色
    uint32_t readColor(uint32_t idx) const { return read<uint32_t>(idx, VertexSemantic::Color0); }
...
public:
    ...
    void write(uint32_t vertexIndex, VertexSemantic sem, const T& value) {
        ...
        if (!attr || vertexIndex >= max_vertices_ || sizeof(T) > attr->size())
            return;
        auto offset = static_cast<size_t>(vertexIndex) * layout_.stride() + attr->offset;
    ...
    void setColor(uint32_t idx, uint32_t packedRGBA) { write(idx, VertexSemantic::Color0, packedRGBA); }

    void setColor(uint32_t idx, uint8_t r, uint8_t g, uint8_t b, uint8_t a = 255) {
        setColor(idx, packColor(r, g, b, a));
    }

    void setPickId(uint32_t idx, uint32_t id) { write(idx, VertexSemantic::PickId, id); }
    ...
    std::span<std::byte> mutableData() { return { reinterpret_cast<std::byte*>(buffer_.data()), buffer_.size() }; }
