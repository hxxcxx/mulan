...
struct CORE_API ArchiveError {
    ...
    static ArchiveError make(Code code, std::string_view msg) { return ArchiveError{ code, std::string(msg), {} }; }

    static ArchiveError missingKey(std::string_view key) {
        return ArchiveError{ Code::MissingKey, std::string("Missing required key: ") + std::string(key), {} };
    }

    static ArchiveError typeMismatch(std::string_view expected, std::string_view actual) {
        return ArchiveError{ Code::TypeMismatch,
                             std::string("Type mismatch: expected ") + std::string(expected) + " but got " +
                                     std::string(actual),
                             {} };
    }
    ...
    static ArchiveError versionTooNew(uint32_t found, uint32_t supported) {
        return ArchiveError{ Code::VersionTooNew,
                             "Data version " + std::to_string(found) + " exceeds supported version " +
                                     std::to_string(supported),
                             {} };
    }

    static ArchiveError outOfMemory(size_t size) {
        return ArchiveError{ Code::OutOfMemory, "Suspicious array size: " + std::to_string(size), {} };
    }
};
