...
DX11SwapChain::DX11SwapChain(const SwapChainDesc& desc, ID3D11Device* device, IDXGIFactory2* factory,
    ...
    : m_desc(desc), m_device(device), m_ctx(ctx), m_renderConfig(renderConfig) {
    ...
    HRESULT hr = factory->CreateSwapChainForHwnd(device, hwnd, &scDesc, nullptr, nullptr, &m_swapChain);
    DX11_CHECK(hr);
