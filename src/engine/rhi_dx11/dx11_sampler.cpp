...
static D3D11_FILTER toDX11Filter(SamplerFilter min, SamplerFilter mag, SamplerFilter mip, bool anisotropy,
    ...
    if (min == SamplerFilter::Nearest && mag == SamplerFilter::Linear && mip == SamplerFilter::Nearest)
        return comparison ? D3D11_FILTER_COMPARISON_MIN_POINT_MAG_LINEAR_MIP_POINT
...
DX11Sampler::DX11Sampler(const SamplerDesc& desc, ID3D11Device* device) : m_desc(desc) {
    ...
    d3dDesc.Filter =
            toDX11Filter(desc.minFilter, desc.magFilter, desc.mipFilter, desc.anisotropyEnable, desc.compareEnable);
