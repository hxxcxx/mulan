...
void DX11CommandList::setScissorRect(const ScissorRect& rect) {
    D3D11_RECT d3dRect = { rect.x, rect.y, rect.x + rect.width, rect.y + rect.height };
...
void DX11CommandList::setIndexBuffer(Buffer* buffer, uint32_t offset, IndexType type) {
    auto* dx11Buf = static_cast<DX11Buffer*>(buffer);
    DXGI_FORMAT fmt = (type == IndexType::UInt16) ? DXGI_FORMAT_R16_UINT : DXGI_FORMAT_R32_UINT;
...
void DX11CommandList::beginRenderPass(const RenderPassBeginInfo& info) {
    ...
    if (info.depthAttachment.target) {
        ...
        if (info.depthAttachment.loadAction == LoadAction::Clear) {
            m_ctx->ClearDepthStencilView(dsv, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, info.clearDepth,
                                         info.clearStencil);
        }
    }
