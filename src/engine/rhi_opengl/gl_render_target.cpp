...
GLRenderTarget::GLRenderTarget(const RenderTargetDesc& desc) : m_desc(desc) {
    createResources();
}
...
void GLRenderTarget::createResources() {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, toGLInternalFormat(m_desc.colorFormat), static_cast<GLsizei>(m_desc.width),
                 static_cast<GLsizei>(m_desc.height), 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
    ...
    TextureDesc colorDesc =
            TextureDesc::renderTarget(m_desc.width, m_desc.height, m_desc.colorFormat, "GLOffscreenColor");
    ...
    if (m_desc.hasDepth) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, internalFmt, static_cast<GLsizei>(m_desc.width),
                     static_cast<GLsizei>(m_desc.height), 0, pixelFmt, pixelType, nullptr);
        ...
        TextureDesc depthDesc =
                TextureDesc::depthStencil(m_desc.width, m_desc.height, m_desc.depthFormat, "GLOffscreenDepth");
    ...
    if (m_desc.hasDepth && depthTex) {
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, depthAttach, GL_TEXTURE_2D, depthTex, 0);
    }
    ...
    std::fprintf(stdout, "[GLRenderTarget] Created FBO %u (%ux%u color=%u depth=%u)\n", m_fbo, m_desc.width,
                 m_desc.height, colorTex, depthTex);
}
