...
core::Result<std::unique_ptr<DX12PipelineState>>
    ...
    , device_(device)
{
    ...
    DXGI_FORMAT rtFormat = (desc_.colorTargetCount > 0)
        ? toDXGIFormat(desc_.colorFormats[0])
        : DXGI_FORMAT_UNKNOWN;
    DXGI_FORMAT dsFormat = desc_.depthEnable
...
void DX12PipelineState::build(DXGI_FORMAT rtFormat, DXGI_FORMAT dsFormat) {
    ...
    if (desc_.vs) vsBytecode = static_cast<DX12Shader*>(desc_.vs)->byteCode();
    if (desc_.ps) psBytecode = static_cast<DX12Shader*>(desc_.ps)->byteCode();
    ...
    depthStencil.DepthWriteMask   = desc_.depthStencil.depthWrite
    ...
    HRESULT hr = device_->CreateGraphicsPipelineState(
        &psoDesc, IID_PPV_ARGS(&pipeline_));
    if (FAILED(hr)) {
