...
DX12Texture::DX12Texture(const TextureDesc& desc, ID3D12Device* device, D3D12_RESOURCE_STATES initialState)
    ...
    if (desc.usage & TextureUsageFlags::RenderTarget) {
        ...
        memcpy(clearVal.Color,
               desc_.format == TextureFormat::Unknown ? std::array<float, 4>{ 0.f, 0.f, 0.f, 1.f }.data()
                                                      : std::array<float, 4>{ 0.15f, 0.15f, 0.15f, 1.f }.data(),
               sizeof(float) * 4);
        pClearVal = &clearVal;
...
void DX12Texture::createSRVIfNeeded(ID3D12Device* device) {
    ...
    srvDesc.Format = isDepthFormat(desc_.format) ? toSRVFormat(desc_.format) : toDXGIFormat(desc_.format);
