...
public:
    ...
    Result<void> uploadBuffer(DX12Buffer* dst, const void* data, uint32_t size, uint32_t dstOffset = 0);

    /// 上传像素数据到纹理：staging 拷贝 + 资源状态转到 PIXEL_SHADER_RESOURCE。
    /// 同步等待 GPU 完成。仅支持单 mip、非压缩颜色格式。
    Result<void> uploadTexture(DX12Texture* dst, const TextureUploadDesc& upload);
...
private:
    ...
    Result<void> submitIfNotBatching();
