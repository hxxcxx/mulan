...
std::unique_ptr<PipelineState> createBakePSO(RHIDevice& device,
                                             Shader* vs, Shader* fs,
...
bool IBLPipeline::bake(RHIDevice& device, const std::string& hdrPath) {
    ...
    auto make2D = [](RHIDevice& dev, uint32_t width, uint32_t height,
                     TextureFormat fmt, const char* name) -> std::unique_ptr<Texture> {
        TextureDesc d;
        d.name      = name;
        d.format    = fmt;
    ...
    // 5a. irradiance
    {
        ...
        if (!bgR) return false;
        IBLParamsGPU params; params.sampleCount = 64;
        cmd->updateBuffer(paramUBO.get(), 0, sizeof(params), &params);
        blitToSlice(*cmd, *pso, **bgR, *irradiance_, TextureFormat::RGBA16_Float,
                    0, 0, kIrradianceW, kIrradianceH, true);
    }
    // 5b. prefilter（单档 roughness=0.5）
    {
        ...
        auto bgR = device.createBindGroup(pso->bindGroupLayout(), bgd);
        if (!bgR) return false;
        IBLParamsGPU params; params.sampleCount = 64; params.roughness = 0.5f;
        cmd->updateBuffer(paramUBO.get(), 0, sizeof(params), &params);
        blitToSlice(*cmd, *pso, **bgR, *prefilter_, TextureFormat::RGBA16_Float,
                    0, 0, kPrefilterW, kPrefilterH, true);
    }
    // 5c. BRDF LUT（不采样外部纹理，但 binding 1/2 仍需占位）
    {
        ...
        auto bgR = device.createBindGroup(pso->bindGroupLayout(), bgd);
        if (!bgR) return false;
        IBLParamsGPU params; params.sampleCount = 512;
        cmd->updateBuffer(paramUBO.get(), 0, sizeof(params), &params);
        blitToSlice(*cmd, *pso, **bgR, *brdf_lut_, TextureFormat::RG16_Float,
                    0, 0, kBrdfLUTSize, kBrdfLUTSize, true);
    }
    ...
    std::fprintf(stderr, "[IBL] bake OK (irradiance=%ux%u prefilter=%ux%u lut=%ux%u)\n",
                 kIrradianceW, kIrradianceH, kPrefilterW, kPrefilterH,
