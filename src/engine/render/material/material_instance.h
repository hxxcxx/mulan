...
public:
    ...
    /// 原始覆盖材质（不含模板合并）
    const Material& overrides() const { return overrides_; }

    uint32_t templateId()   const { return template_id_; }
    bool    isFromTemplate() const { return template_id_ != 0; }
    bool    isDirty()        const { return dirty_; }
    MaterialOverrideFlags overrideFlags() const { return override_flags_; }
    ...
    bool isOverridden(MaterialOverrideFlags flag) const {
        return hasFlag(override_flags_, flag);
    }

private:
    void rebuildFromTemplate();

    Material             material_;       // 最终解析后的参数（覆盖 + 模板合并）
    Material             overrides_;      // 仅覆盖值（其他字段默认值）
    MaterialOverrideFlags override_flags_ = MaterialOverrideFlags::None;
    uint32_t             template_id_ = 0; // 模板 ID（0 表示无模板）
    bool                 dirty_ = true;   // 是否需要重新上传 UBO

    // 保存高亮前的状态用于 clearHighlight
    bool                 highlighted_ = false;
    Material             saved_material_;  // 高亮前保存
};
