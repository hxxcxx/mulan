...
public:
    ...
    Texture* atlasTexture() const { return texture_.get(); }

    /// 获取采样器
    Sampler* atlasSampler() const { return sampler_.get(); }

    /// 基准字号（用于排版缩放计算）
    float baseFontSize() const { return base_font_size_; }

    /// MSDF 像素范围（传给 shader 的 pxRange）
    float pxRange() const { return px_range_; }
