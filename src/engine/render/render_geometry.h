...
struct RenderGeometry {
    ...
    // 便捷：索引数据总字节数
    uint32_t indexDataSize() const { return static_cast<uint32_t>(indexBytes.size()); }
    // 是否有索引
...
struct RenderItem {
    ...
    void computeTransparentSortKey(const math::Vec3& cameraPos) {
        ...
        sortKey = (static_cast<uint64_t>(0xFFFF - materialIndex) << 32) | (0xFFFFFFFFu - distBits);  // 翻转使远距优先
    }
};
...
public:
    ...
    void sort(const math::Vec3& cameraPos) {
        ...
        auto mid2 = std::stable_partition(mid1, items_.end(), [](const RenderItem& a) { return a.isEdge; });
        ...
        std::sort(items_.begin(), mid1, [](const RenderItem& a, const RenderItem& b) { return a.sortKey < b.sortKey; });
    ...
    /// 不透明子范围
    std::span<const RenderItem> opaqueItems() const { return { items_.data(), opaque_split_ }; }
