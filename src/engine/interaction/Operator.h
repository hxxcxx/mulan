...
public:
    ...
    bool isFinished()  const { return m_state == State::Finished || m_state == State::Cancelled; }
    bool isCompleted() const { return m_state == State::Finished; }

    /// 注册完成回调（由 Viewport 在 push 时自动连接到 popOperator）
    /// 在 Operator 完成后、由 Viewport 于 Operator 外部触发（非 finish() 内）。
    void onFinish(FinishCallback cb) { m_onFinish = std::move(cb); }
    ...
    const FinishCallback& finishHook() const { return m_onFinish; }
...
protected:
    ...
    void finish(bool completed = true) {
        if (m_state == State::Finished || m_state == State::Cancelled) return;
        m_state = completed ? State::Finished : State::Cancelled;
    }
