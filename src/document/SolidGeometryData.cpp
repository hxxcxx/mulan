...
void SolidGeometryData::setShape(const TopoDS_Shape& shape) {
    ...
    m_impl->shape = shape;
    m_impl->faceValid = false;
    m_impl->edgeValid = false;
}
...
engine::Mesh SolidGeometryData::faceMesh() const {
    std::lock_guard lock(m_impl->mtx);
    if (!m_impl->faceValid) {
        m_impl->cachedFaceMesh = triangulate();
        m_impl->faceValid = true;
    }
    return m_impl->cachedFaceMesh;
}

engine::Mesh SolidGeometryData::triangulate() const {
    auto& shape = m_impl->shape;
...
engine::Mesh SolidGeometryData::edgeMesh() const {
    std::lock_guard lock(m_impl->mtx);
    if (!m_impl->edgeValid) {
        m_impl->cachedEdgeMesh = extractEdges();
        m_impl->edgeValid = true;
    }
    return m_impl->cachedEdgeMesh;
}
