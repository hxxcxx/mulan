...
struct AABB3 {
    ...
    Point3 max{ -std::numeric_limits<double>::max(), -std::numeric_limits<double>::max(),
                -std::numeric_limits<double>::max() };
    ...
    bool isEmpty(const Tolerance& tol = defaultTolerance()) const {
        return min.x > max.x + tol.lengthEps || min.y > max.y + tol.lengthEps || min.z > max.z + tol.lengthEps;
    }
    ...
    bool contains(const Point3& p, const Tolerance& tol = defaultTolerance()) const {
        return p.x >= min.x - tol.lengthEps && p.x <= max.x + tol.lengthEps && p.y >= min.y - tol.lengthEps &&
               p.y <= max.y + tol.lengthEps && p.z >= min.z - tol.lengthEps && p.z <= max.z + tol.lengthEps;
    }
    ...
    AABB3 transformed(const Mat4& m) const {
        ...
        AABB3 r;
        for (int i = 0; i < 8; ++i) {
            Point3 corner((i & 1) ? max.x : min.x, (i & 2) ? max.y : min.y, (i & 4) ? max.z : min.z);
...
struct AABB2 {
    Point2 min{ std::numeric_limits<double>::max(), std::numeric_limits<double>::max() };
    Point2 max{ -std::numeric_limits<double>::max(), -std::numeric_limits<double>::max() };
    ...
    AABB2 transformed(const Mat3& m) const {
        if (isEmpty())
            return empty();
        AABB2 r;
        Point2 corners[4] = { { min.x, min.y }, { max.x, min.y }, { max.x, max.y }, { min.x, max.y } };
