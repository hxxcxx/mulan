...
struct Line3 {
    ...
    Point3 pointAt(double t) const { return origin + direction * t; }
...
struct Segment3 {
    ...
    constexpr Segment3(const Point3& s, const Point3& e) : start(s), end(e) {}
...
struct Ray3 {
    ...
    Point3 pointAt(double t) const { return origin + direction * t; }  // t≥0
...
struct Line2 {
    ...
    Point2 pointAt(double t) const { return origin + direction * t; }
    ...
    Line2 transformed(const Mat3& m) const {
        ...
        return Line2(Point2(o3.x, o3.y), Vec2(d3.x, d3.y).normalized());
    }
};
...
struct Segment2 {
    ...
    constexpr Segment2(const Point2& s, const Point2& e) : start(s), end(e) {}

    Vec2 direction() const { return end - start; }
    double length() const { return direction().length(); }
    double lengthSq() const { return direction().lengthSq(); }
    Point2 pointAt(double t) const { return start + direction() * t; }

    /// 经 2D 矩阵变换（两端点按点变换）
    Segment2 transformed(const Mat3& m) const {
        Vec3 s3 = m * Vec3(start.x, start.y, 1.0);
        Vec3 e3 = m * Vec3(end.x, end.y, 1.0);
        return Segment2(Point2(s3.x, s3.y), Point2(e3.x, e3.y));
    }
};
