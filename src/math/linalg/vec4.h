...
template <typename T>
constexpr Vec4T<T> operator*(const Vec4T<T>& a, T s) {
    return Vec4T<T>(a.x * s, a.y * s, a.z * s, a.w * s);
}
template <typename T>
constexpr Vec4T<T> operator*(T s, const Vec4T<T>& a) {
    return a * s;
}
template <typename T>
constexpr Vec4T<T> operator/(const Vec4T<T>& a, T s) {
...
template <typename T>
template <typename U>
constexpr Vec3T<T>::Vec3T(const Vec4T<U>& v) : x(static_cast<T>(v.x)), y(static_cast<T>(v.y)), z(static_cast<T>(v.z)) {
}
