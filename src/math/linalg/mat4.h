...
template <typename T>
struct Mat4T {
    Vec4T<T> cols[4]{ Vec4T<T>(T(1), T(0), T(0), T(0)), Vec4T<T>(T(0), T(1), T(0), T(0)),
                      Vec4T<T>(T(0), T(0), T(1), T(0)), Vec4T<T>(T(0), T(0), T(0), T(1)) };

    constexpr Mat4T() = default;
    explicit constexpr Mat4T(T diagonal)
        : cols{ Vec4T<T>(diagonal, T(0), T(0), T(0)), Vec4T<T>(T(0), diagonal, T(0), T(0)),
                Vec4T<T>(T(0), T(0), diagonal, T(0)), Vec4T<T>(T(0), T(0), T(0), diagonal) } {}
    constexpr Mat4T(const Vec4T<T>& c0, const Vec4T<T>& c1, const Vec4T<T>& c2, const Vec4T<T>& c3)
        : cols{ c0, c1, c2, c3 } {}

    explicit constexpr Mat4T(const Mat3T<T>& m)
        : cols{ Vec4T<T>(m[0], T(0)), Vec4T<T>(m[1], T(0)), Vec4T<T>(m[2], T(0)), Vec4T<T>(T(0), T(0), T(0), T(1)) } {}
    ...
    static constexpr Mat4T translate(const Vec3T<T>& t) {
        return Mat4T(Vec4T<T>(T(1), T(0), T(0), T(0)), Vec4T<T>(T(0), T(1), T(0), T(0)),
                     Vec4T<T>(T(0), T(0), T(1), T(0)), Vec4T<T>(t.x, t.y, t.z, T(1)));
    }
    ...
    static Mat4T rotation(const Vec3T<T>& axis, T rad) {
        ...
        T x = n.x, y = n.y, z = n.z;
        return Mat4T(Vec4T<T>(t * x * x + c, t * x * y + s * z, t * x * z - s * y, T(0)),
    ...
    static Mat4T rotationX(T rad) {
        ...
        return Mat4T(Vec4T<T>(T(1), T(0), T(0), T(0)), Vec4T<T>(T(0), c, s, T(0)), Vec4T<T>(T(0), -s, c, T(0)),
                     Vec4T<T>(T(0), T(0), T(0), T(1)));
    }
    static Mat4T rotationY(T rad) {
        T c = std::cos(rad), s = std::sin(rad);
        return Mat4T(Vec4T<T>(c, T(0), -s, T(0)), Vec4T<T>(T(0), T(1), T(0), T(0)), Vec4T<T>(s, T(0), c, T(0)),
                     Vec4T<T>(T(0), T(0), T(0), T(1)));
    }
    static Mat4T rotationZ(T rad) {
        T c = std::cos(rad), s = std::sin(rad);
        return Mat4T(Vec4T<T>(c, s, T(0), T(0)), Vec4T<T>(-s, c, T(0), T(0)), Vec4T<T>(T(0), T(0), T(1), T(0)),
                     Vec4T<T>(T(0), T(0), T(0), T(1)));
    }

    static constexpr Mat4T scale(const Vec3T<T>& s) {
        return Mat4T(Vec4T<T>(s.x, T(0), T(0), T(0)), Vec4T<T>(T(0), s.y, T(0), T(0)), Vec4T<T>(T(0), T(0), s.z, T(0)),
                     Vec4T<T>(T(0), T(0), T(0), T(1)));
    }
    ...
    static Mat4T perspective(T fovY, T aspect, T zNear, T zFar) {
        ...
        T zRange = zNear - zFar;
        return Mat4T(Vec4T<T>(f / aspect, T(0), T(0), T(0)), Vec4T<T>(T(0), f, T(0), T(0)),
    ...
    static Mat4T ortho(T left, T right, T bottom, T top, T zNear, T zFar) {
        ...
        T tb = T(1) / (top - bottom);
        T fn = T(1) / (zNear - zFar);
        return Mat4T(Vec4T<T>(T(2) * rl, T(0), T(0), T(0)), Vec4T<T>(T(0), T(2) * tb, T(0), T(0)),
    ...
    static Mat4T lookAt(const Vec3T<T>& eye, const Vec3T<T>& center, const Vec3T<T>& up) {
        ...
        Vec3T<T> u = s.cross(f);
        return Mat4T(Vec4T<T>(s.x, u.x, -f.x, T(0)), Vec4T<T>(s.y, u.y, -f.y, T(0)), Vec4T<T>(s.z, u.z, -f.z, T(0)),
    ...
    Mat4T transposed() const {
        const Mat4T& m = *this;
        return Mat4T(Vec4T<T>(m[0].x, m[1].x, m[2].x, m[3].x), Vec4T<T>(m[0].y, m[1].y, m[2].y, m[3].y),
                     Vec4T<T>(m[0].z, m[1].z, m[2].z, m[3].z), Vec4T<T>(m[0].w, m[1].w, m[2].w, m[3].w));
    }
...
constexpr Vec4T<T> operator*(const Mat4T<T>& m, const Vec4T<T>& v) {
    return Vec4T<T>(m[0].x * v.x + m[1].x * v.y + m[2].x * v.z + m[3].x * v.w,
...
constexpr Mat3T<T>::Mat3T(const Mat4T<U>& m)
    : cols{ Vec3T<T>(m[0].x, m[0].y, m[0].z), Vec3T<T>(m[1].x, m[1].y, m[1].z), Vec3T<T>(m[2].x, m[2].y, m[2].z) } {
}
