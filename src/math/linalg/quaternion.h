...
struct QuatT {
    ...
    static QuatT fromTwoVectors(Vec3T<T> from, Vec3T<T> to) {
        ...
        if (d >= T(1))
            return identity();  // 平行
        if (d <= T(-1)) {       // 反向，任选正交轴
            Vec3T<T> axis = std::abs(from.x) < T(0.9) ? Vec3T<T>::unitX().cross(from) : Vec3T<T>::unitY().cross(from);
    ...
    QuatT operator*(const QuatT& q) const {
        return QuatT(w * q.w - x * q.x - y * q.y - z * q.z, w * q.x + x * q.w + y * q.z - z * q.y,
    ...
    Mat3T<T> toMat3() const {
        ...
        return Mat3T<T>(Vec3T<T>(T(1) - T(2) * (yy + zz), T(2) * (xy + wz), T(2) * (xz - wy)),
                        Vec3T<T>(T(2) * (xy - wz), T(1) - T(2) * (xx + zz), T(2) * (yz + wx)),
                        Vec3T<T>(T(2) * (xz + wy), T(2) * (yz - wx), T(1) - T(2) * (xx + yy)));
    }
    Mat4T<T> toMat4() const {
        Mat3T<T> r = toMat3();
        return Mat4T<T>(Vec4T<T>(r[0].x, r[0].y, r[0].z, T(0)), Vec4T<T>(r[1].x, r[1].y, r[1].z, T(0)),
                        Vec4T<T>(r[2].x, r[2].y, r[2].z, T(0)), Vec4T<T>(T(0), T(0), T(0), T(1)));
    }
    ...
    static QuatT fromMat3(const Mat3T<T>& m) {
        ...
        if (trace > T(0)) {
            ...
            return QuatT(T(0.25) * s, (m[1].z - m[2].y) / s, (m[2].x - m[0].z) / s, (m[0].y - m[1].x) / s).normalized();
        } else if (m[0].x > m[1].y && m[0].x > m[2].z) {
            T s = std::sqrt(T(1) + m[0].x - m[1].y - m[2].z) * T(2);  // s = 4*x
            return QuatT((m[1].z - m[2].y) / s, T(0.25) * s, (m[1].x + m[0].y) / s, (m[2].x + m[0].z) / s).normalized();
        } else if (m[1].y > m[2].z) {
            T s = std::sqrt(T(1) + m[1].y - m[0].x - m[2].z) * T(2);  // s = 4*y
            return QuatT((m[2].x - m[0].z) / s, (m[1].x + m[0].y) / s, T(0.25) * s, (m[2].y + m[1].z) / s).normalized();
        } else {
            T s = std::sqrt(T(1) + m[2].z - m[0].x - m[1].y) * T(2);  // s = 4*z
            return QuatT((m[0].y - m[1].x) / s, (m[2].x + m[0].z) / s, (m[2].y + m[1].z) / s, T(0.25) * s).normalized();
        }
    }
};
...
QuatT<T> slerp(const QuatT<T>& a, QuatT<T> b, T t) {
    ...
    if (cosTheta > T(0.9995)) {
        // 接近共线，线性插值并归一化即可
        return QuatT<T>(a.w + (b.w - a.w) * t, a.x + (b.x - a.x) * t, a.y + (b.y - a.y) * t, a.z + (b.z - a.z) * t)
                .normalized();
    }
    ...
    return QuatT<T>(a.w * k0 + b.w * k1, a.x * k0 + b.x * k1, a.y * k0 + b.y * k1, a.z * k0 + b.z * k1);
}
