...
template <typename T>
struct Mat3T {
    Vec3T<T> cols[3]{ Vec3T<T>(T(1), T(0), T(0)), Vec3T<T>(T(0), T(1), T(0)), Vec3T<T>(T(0), T(0), T(1)) };

    constexpr Mat3T() = default;
    explicit constexpr Mat3T(T diagonal)
        : cols{ Vec3T<T>(diagonal, T(0), T(0)), Vec3T<T>(T(0), diagonal, T(0)), Vec3T<T>(T(0), T(0), diagonal) } {}
    constexpr Mat3T(const Vec3T<T>& c0, const Vec3T<T>& c1, const Vec3T<T>& c2) : cols{ c0, c1, c2 } {}

    template <typename U>
    explicit constexpr Mat3T(const Mat3T<U>& m) : cols{ Vec3T<T>(m[0]), Vec3T<T>(m[1]), Vec3T<T>(m[2]) } {}
    ...
    static Mat3T rotation(const Vec3T<T>& axis, T rad) {
        ...
        T x = n.x, y = n.y, z = n.z;
        // 列主序
        return Mat3T(Vec3T<T>(t * x * x + c, t * x * y + s * z, t * x * z - s * y),
    ...
    static Mat3T rotationZYX(T rz, T ry, T rx) {
        return rotation(Vec3T<T>(T(0), T(0), T(1)), rz) * rotation(Vec3T<T>(T(0), T(1), T(0)), ry) *
               rotation(Vec3T<T>(T(1), T(0), T(0)), rx);
    }

    static constexpr Mat3T scale(T sx, T sy, T sz) {
        return Mat3T(Vec3T<T>(sx, T(0), T(0)), Vec3T<T>(T(0), sy, T(0)), Vec3T<T>(T(0), T(0), sz));
    }
    static constexpr Mat3T scale(const Vec3T<T>& s) { return scale(s.x, s.y, s.z); }

    Mat3T transposed() const {
        return Mat3T(Vec3T<T>(cols[0].x, cols[1].x, cols[2].x), Vec3T<T>(cols[0].y, cols[1].y, cols[2].y),
    ...
    Mat3T inverse() const {
        ...
        const auto& c = cols[2];
        return Mat3T(
                Vec3T<T>((b.y * c.z - b.z * c.y) * inv, (a.z * c.y - a.y * c.z) * inv, (a.y * b.z - a.z * b.y) * inv),
                Vec3T<T>((b.z * c.x - b.x * c.z) * inv, (a.x * c.z - a.z * c.x) * inv, (a.z * b.x - a.x * b.z) * inv),
                Vec3T<T>((b.x * c.y - b.y * c.x) * inv, (a.y * c.x - a.x * c.y) * inv, (a.x * b.y - a.y * b.x) * inv));
    }
};

template <typename T>
constexpr Vec3T<T> operator*(const Mat3T<T>& m, const Vec3T<T>& v) {
    return Vec3T<T>(m[0].x * v.x + m[1].x * v.y + m[2].x * v.z, m[0].y * v.x + m[1].y * v.y + m[2].y * v.z,
