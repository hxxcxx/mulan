...
inline Hit3 intersect(const Ray3& ray, const AABB3& box, const Tolerance& tol = defaultTolerance()) {
    if (box.isEmpty(tol))
        return Hit3::miss();
...
/// 射线 - 平面
inline Hit3 intersect(const Ray3& ray, const Plane3& plane, const Tolerance& tol = defaultTolerance()) {
...
inline Hit3 intersect(const Ray3& ray, const Point3& v0, const Point3& v1, const Point3& v2, Vec3* bc = nullptr,
...
/// 线段 - 平面：返回线段参数 t∈[0,1]
inline Hit3 intersect(const Segment3& seg, const Plane3& plane, const Tolerance& tol = defaultTolerance()) {
...
inline bool intersect(const Segment2& a, const Segment2& b, double* sa = nullptr, double* sb = nullptr) {
    Vec2 d1 = a.direction();
    Vec2 d2 = b.direction();
...
inline Hit3 intersect(const Line3& la, const Line3& lb, const Tolerance& tol = defaultTolerance()) {
    Vec3 u = la.direction;
    Vec3 v = lb.direction;
...
/// 直线 - 平面
inline Hit3 intersect(const Line3& line, const Plane3& plane, const Tolerance& tol = defaultTolerance()) {
